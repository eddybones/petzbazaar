...
$sql = "SELECT quartz, shop_slots FROM users WHERE id=". $_SESSION["userID"];
...
if (mysqli_num_rows($userresult)==1 && mysqli_num_rows($stockresult)==1) {
    ...
    if ($user["quartz"] >= $stock["price"]) {
        ...
        $sql = "UPDATE users Set quartz = quartz - ". $stock["price"] .", shop_slots = shop_slots+1 WHERE id=". $_SESSION["userID"];
        $statement = mysqli_prepare($conn, $sql);
        mysqli_stmt_execute($statement);
        ...
        $message = "Shop slot upgrade complete. You have gained 1 shop slot.</a>";
        $subject = "Your Purchase";
        sendMessage(0, $sender, $recipient, $message, $subject);
      

        $_SESSION["message"] = "Upgrade Complete!";
